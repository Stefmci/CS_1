...
class Serializable(ABC):
    ...
    def __init__(self, id, creation_date, dateime = None, last_update : datetime = None):
        self.id = id
    ...
    def store_data(self):
        ...
        result = self.db_connector.upsert(self.__dict__, DeviceQuery.id == self.id)
        if result:
            print("Data updated.")
        else:
            print("Data inserted.")
    ...
    def delete(self):
        print("Deleting data...")
        DeviceQuery = Query()
        if self.db_connector.remove(DeviceQuery.id == self.id):
            print("Data deleted.")
        else:
